...
def Aumento_precio(diccionario:dict)->dict:
        ...
        return diccionario
def Agregar_insumos(diccionario:dict):
    lista = []
    seguir = True
    agregar = False
    marca_valida = True
    ...
    while seguir:
        ...
        while id_valida == True:
            ...
            for id in diccionario:
                ...
                if ID == a:
                    agregar = True
            if agregar == True:
                print("ese Id ya existe intente con otro: ")
            else:
                id_valida = False
        nombre = input("ingresa el nombre que tendra el nuevo producto: ")
        while verifica:
            TXT = open(r"C:\Users\Aaron\OneDrive\Escritorio\Primer parcial real\Primer_Parcial\Marcas.txt", "r",encoding='utf-8')
        ...
        caracteristica1 = input("Ingrese una caracteristica :")
        espacio1 = " "
        caracteristica2 = input("Ingrese otra caracteristica: ")
        caracteristica3 = input("Ingrese otra caracteristica: ")
        
        respuesta = input("deseas continuar")
        lista.append({'ID':ID,'nombre':nombre,'marca':marca,'precio':precio,'caracteristicas':caracteristica1+espacio1+caracteristica2+espacio1+caracteristica3})
        if respuesta == "no":
            seguir = False
            ...
            **** Menu de opciones ***
            ...
            match(Numero):
                ...
                case 10:
                    ...
                    Lista_agregados = Agregar_insumos(diccionario)
                    print(Lista_agregados)
                    archivito = input("Decida el formato de exportacion : 1 = Json o 2 = csv")
                    nombre = input("de el nombre del archivo")
                    if archivito == "1":
                        with open(nombre,"w",encoding='utf-8') as agregados_json:
                            for i in diccionario:
                                a = str({i['ID'],i['marca'],i['nombre'],i['precio'],i['caracteristicas']})
                                agregados_json.write(a)
                                agregados_json.write("\n")
                            for t in Lista_agregados:
                                b = str({t['ID'],t['marca'],t['nombre'],t['precio'],t['caracteristicas']})
                                agregados_json.write(b)
                                agregados_json.write("\n")
                        agregados_json.close
                        agregados_json = open(nombre,"r",encoding='utf-8')
                        for i in agregados_json:
                            print(i)
                    
                    if archivito == "2":
                        print("entra")
                        with open(nombre, "w", newline="",encoding='utf-8') as archivo_csv2:
                            escribir = csv.writer(archivo_csv2)
                            escribir.writerow(['ID','nombre','marca','precio','caracteristica'])
                            for i in diccionario:
                                    archivo_csv2.write(i['ID'],)
                                    archivo_csv2.write(i['nombre'])
                                    archivo_csv2.write(i['marca'])
                                    archivo_csv2.write(i['precio'])
                                    archivo_csv2.write(i['caracteristicas'])
                                    archivo_csv2.write("\n")
                            for t in Lista_agregados:
                                b = str({t['ID'],t['marca'],t['nombre'],t['precio'],t['caracteristicas']})
                                archivo_csv2.write(str(t['ID']))
                                archivo_csv2.write(str(t['nombre']))
                                archivo_csv2.write(str(t['marca']))
                                archivo_csv2.write(str(t['precio']))
                                archivo_csv2.write(str(t['caracteristicas']))
                                archivo_csv2.write("\n")
                        archivo_csv2.close
                        archivo_csv2 = open(nombre,"r",encoding='utf-8')
                        for i in archivo_csv2:
                            print(iter)
